...
export const updateTaskInDb = async (
  ...
  try {
    ...
    console.error("updateTaskInDb error:", error);
    throw error;
  }
};
...
export const reorderTasksInDb = async (
  ...
  taskOrder: TASKS_SORT_ORDER = "default"
) => {
  try {
    await Axios.post(
      `http://localhost:${config.port}/projects/${projectId}/commands/reorder?taskOrder=${taskOrder}`,
      {
        commands
      }
    );

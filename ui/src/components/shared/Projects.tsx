...
interface IProjectContextValue {
  ...
  updateActiveProjectWithSort: (activeProject: IProject) => void;
  setProjects: any;
  updateProjects: () => void;
  ...
  updateTask: (
    projectId: string,
    taskId: string,
    taskUpdates: Partial<IProjectCommand>
  ) => any;
...
function ProjectsProvider(props: IProjectsProviderProps) {
  ...
  const updateActiveProjectWithSort = (newActiveProject: IProject) => {
    let taskOrder: TASKS_SORT_ORDER = newActiveProject.taskOrder || "name-asc";
    const reorderedProject: IProject = recorderProjectTasks(
      newActiveProject,
      taskOrder
    );
    setActiveProject(reorderedProject);
  };
  ...
  const reorderTasks = React.useCallback(
    (
      ...
      const reorderTasksFn = async () => {
        await reorderTasksInDb(config, projectId, commands, taskOrder);
        const currentProjectIndex = projects.findIndex(
          x => x._id === projectId
        );
        const projectWithThisTask = projects[currentProjectIndex];
  ...
  const updateTask = React.useCallback(
    (
      ...
      const updateTaskFn = async () => {
        await updateTaskInDb(config, projectId, taskId, taskUpdates);
        const currentProjectIndex = projects.findIndex(
          x => x._id === projectId
        );
        ...
        const currentTaskIndex = projectWithThisTask.commands.findIndex(
          x => x._id === taskId
        );

        const taskWithThisId = projectWithThisTask.commands[currentTaskIndex];

        const updatedTask: IProjectCommand = {
          ...taskWithThisId,
          ...taskUpdates
        };

        const updatedCommands = projectWithThisTask.commands.map(task => {
          if (task._id === taskId) {
            return updatedTask;
          }
          return task;
        ...
        if (projectWithThisTask) {
          ...
          if (
            taskUpdates.lastExecutedAt !== undefined ||
            taskUpdates.lastExecutedAt !== null
          ) {
            // If lasExecutedAt is changed, make change sortOrder to default
            updatedProject = {
              ...updatedProject,
              taskOrder: "default"
            };
